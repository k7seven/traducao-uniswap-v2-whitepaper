...
No Uniswap V1, um usuário comprando ABC com XYZ precisa enviar XYZ para o contrato antes de poder receber ABC. Isso é uma inconveniência se os usuários precisam de ABC que estão comprando para obter XYZ que estão pagando. Por exemplo, o usuário pode estar usando ABC para comprar XYZ em algum outro contrato para fazer arbitragem a um preço diferente do Uniswap, ou eles podem estar desfazendo de uma posição no Maker ou Compound ao vender o colateral para pagar o Uniswap.

O Uniswap V2 adiciona uma nova funcionalidade que permite ao usuário receber e usar um ativo antes de pagar por ele, desde que eles façam o pagamento dentro da mesma transação atômica. A função de swap faz uma chamada para um contrato de retorno de chamada opcional especificado pelo usuário entre a transferência dos tokens solicitados pelo usuário e a execução da troca. Assim que o retorno de chamada é concluído, o contrato verifica os novos saldos e confirma se a troca está satisfeita (depois de ajustar para as taxas dos valores pagos). Se o contrato não tem fundos suficientes, ele reverte a transação inteira.

Um usuário também pode reembolsar o pool do Uniswap usando o mesmo token, em vez de completar a troca. Isso é efetivamente o mesmo que permitir que qualquer pessoa faça um flash-empréstimo de qualquer ativo armazenado em um pool do Uniswap (pelas mesmas taxas de 0,30% que o Uniswap cobra para as negociações).
